...
// Possible forward pipe that extends the ForwardPipe interface which is used in the network to do forward propagation.
...
// Applies a winograd transformation on the input data "in" and stores the transformed data in the vector V.
...
// Performs matrix multiplication using the data transofrmed by the winograd transformation.
void CPUPipe::winograd_sgemm(const std::vector<float>& U,
    ...
    for (auto b = 0; b < WINOGRAD_TILE; b++) {
        ...
        C_mat.noalias() =
            ...
            * ConstEigenMatrixMap<float>(U.data() + offset_u, K, C).transpose(); // Part that performs the multiplication.
#endif
    }
}

// Reverses the winograd transformation after the matrix multiplication to obtain the output from the channels.
void CPUPipe::winograd_transform_out(const std::vector<float>& M,
    ...
    // Iterates for every channel in the output and reverses the winograd transformation to obtain the actual output.
    for (auto k = 0; k < K; k++) {
        for (auto block_x = 0; block_x < WTILES; block_x++) {
...
// Takes the inputs and calls the winograd transformation pipeline to get the output of the convolutional layers.
...
// Applies a traditional convolution function.
...
// Applies batch normalization to input vectors.
...
// Does the forwarding in the convolutional network: it applies convolution to the input data,
// applies batch normalization to the output, then for each pair of convolutional layers in the residual tower
// it applies batch normalization to the first convolutional layer output. Then it applies it again
// to the output of the second convolutional layer and finally adds the original input to the output.
// After all this it applies the fully connected convolutional layers to obtain policy and value outputs.
void CPUPipe::forward(const std::vector<float>& input,
    ...
    convolve<1>(Network::OUTPUTS_POLICY, conv_out, m_conv_pol_w, m_conv_pol_b,
                output_pol); // Computes the fully connected convolutional layers.
    convolve<1>(Network::OUTPUTS_VALUE, conv_out, m_conv_val_w, m_conv_val_b,
                output_val);
}

// Sets up the weights, seperating the ones for the policy and the value head from the convolutional layer ones.
