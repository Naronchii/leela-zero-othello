...
OpenCLScheduler<net_t>::OpenCLScheduler() {
    ...
    // For each GPU it creats an OpenCL object and an OpenCL_Network object (the class that uses opencl cores to do neural network operations) then adds it to the vectors to save them.
    for (auto gpu : gpus) {
...
void OpenCLScheduler<net_t>::initialize(const int channels) {
    ...
    // Calculates the number of threads to allocate for each GPU device.
    ...
    for (auto& opencl : m_opencl) {
        ...
        for (auto i = unsigned{0}; i < num_worker_threads; i++) {
            // For each device it launches a thread that executes the batch_worker function with the gnum to determine on which gpu it works on.
            auto t =
...
// Pushes the input layer weights into the network.
template <typename net_t>
void OpenCLScheduler<net_t>::push_input_convolution(
    ...
    for (const auto& opencl_net : m_networks) { // It pushes the same weights and channels into every OpenCL network.
        ...
        // Applies padding to the input weights.
        const auto Upad =
        ...
        opencl_net->push_input_convolution(filter_size, channels, outputs, Upad,
                                           from_float(means),
                                           from_float(variances)); // Pushes the weights into the input layer.
    }
}
...
void OpenCLScheduler<net_t>::push_residual(
    ...
    // It pushes the same weights and channels onto every OpenCL network.
...
// Pushes all the weights into the network by calling all the three functions created above.
template <typename net_t>
...
void OpenCLScheduler<net_t>::batch_worker(const size_t gnum) {
    ...
    // Loops to wait for task to pick up.
