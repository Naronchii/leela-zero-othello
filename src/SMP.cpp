...
void SMP::Lock::lock() {
    ...
    // The thread continues to test if the mutex is available until it is able to acquire it safely.
...
void SMP::Lock::unlock() {
    ...
    assert(m_owns_lock);
    // Resets the value of m_lock to false atomically and with the memory order std::memory_order_release.
