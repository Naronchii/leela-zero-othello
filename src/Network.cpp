...
// Benchmarks the network given a state and an amount of fixed iterations instead of time
...
// Parses the network weights from the weights file given in input as istream 'wtfile'
std::pair<int, int> Network::load_v1_network(std::istream& wtfile) {
    ...
    while (std::getline(wtfile, line)) {
        ...
        if (linecount == 2) {
            auto count = std::distance(std::istream_iterator<std::string>(iss),
                                       ...
                                       // Since there is a bias weight for every filter,
                                       // if you count the number of bias weights you can also
                                       // know how many channels there are per block
    ...
    while (std::getline(wtfile, line)) {
        ...
        if (linecount < plain_conv_wts) {
            ...
            } else if (linecount % 4 == 1) { // Reads convolutional layer biases 
        ...
        } else { // After reading convolutional layer weights
            switch (linecount - plain_conv_wts) {
                ...
                case 3:
                    std::copy(cbegin(weights), cend(weights),
                              begin(m_bn_pol_w2));
                    break;
                case 4: // Checks if the value head weights are consistent with the given state of the board
                    if (weights.size()
                ...
                case 5:
                    std::copy(cbegin(weights), cend(weights),
                              begin(m_ip_pol_b));
                    break;
                case 6: m_fwd_weights->m_conv_val_w = std::move(weights); break; // Handles value convolution weights
...
// Loads the weights file and processes it before calling the load_v1_network function
...
/* Performs batch normalization (BN) on a given data tensor.
Batch normalization is a technique used to normalize the activations of 
each layer in a neural network, typically applied after the linear 
transformation and before the non-linear activation function.
It helps in improving the training speed and stability of the neural network.
*/
...
// Applies the softmax function to the input data, giving percentages of every possible output.
// Likely used in policy head
...
// Checks if the evalutaions for the current board state (or symmetrical board states) is present in cache
...
// A Netresult is a data structure that contains the policy and the winrate for the game state.
// Therefore it contains all the outputs for the network
...
// Function called by get_output
// It produces the Netresult object for the final output of the network,
// gathers features, does forward propagation, does batch normalization on the outputs,
// does calculations with innerproduct and then calls the softmax function
...
// Shows a visual representation of the neural network's policy output for each position
...
// Constructs the input data for the neural network based on the current game state
// and the specified symmetry transformation
std::vector<float> Network::gather_features(const GameState* const state,
    ...
    for (auto h = size_t{0}; h < moves; h++) { // Checks up to 7 previous moves for each player
