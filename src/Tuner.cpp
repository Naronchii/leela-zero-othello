...
// Multiplication of pairs of input matrices.
...
static void sgemmBatched_ref(const std::vector<net_t>& a,
    ...
    // Temporary vectors for the matrix pairs.
    ...
    // Calculate the correct offsets to access the matrices.
...
Parameters Tuner<net_t>::get_parameters_by_int(
    ...
    // For each parameter count the possible choices and multiply them to obtain the configurations.
    auto cfgs = 1;
...
// Returns a string containing the prepocessor definitions.
template <typename net_t>
std::string Tuner<net_t>::parameters_to_defines(const Parameters& p) {
    std::string s;
    // Scan all the elements of the map p.
    for (auto const& x : p) {
...
std::string Tuner<net_t>::parameters_to_string(const Parameters& p) {
    ...
    // Eventually remove the extra space added at the end of the string in the last for cycle.
    if (s.size() > 0) {
        s.resize(s.size() - 1);
    }
    return s;
...
// Calculate the next power of 2 larger than x.
...
static void sgemm_generate_data(std::vector<net_t>& x, const int m, const int n,
                                ...
                                const int n_ceil) {
    // Scan the data batches.
    for (auto batch = 0; batch < batch_size; batch++) {
        // Scan the matrix lines.
...
// Compare the results of the multiplication between matrices with referenced results to evaluate the accuracy of the algorithm.
template <typename net_t>
...
std::vector<Parameters> Tuner<net_t>::build_valid_params() {
    ...
    auto build_from = [this, &valid_params](std::vector<Configurations>& opts,
                                            int tce) {
        auto cfgs = 1;
        // Calculate the number of total possible configurations.
        for (auto c = size_t{0}; c < opts.size(); c++) {
            cfgs *= opts[c].second.size();
        }
        // Generate a complete set of parameters.
...
std::string Tuner<net_t>::tune_sgemm(const int m, const int n, const int k,
    ...
    // Calculate the array sizes which contain the matrices.
    auto at_size =
    ...
    // Number of total operations in floating point.
    ...
    // Load the buffer to memorize the data in the matrices.
    ...
    for (auto& p : valid_params) {
        ...
        try {
            // Tries to ocmpile the OpenCL program with the current definitions.
        ...
        // Calculate work block sizes on current parameters.
        ...
        // Checks if the block sizes have changed.
        ...
        // Set OpenCL kernel arguments with work blocks and data buffer sizes.
        ...
        // Calculate local and global sizes for the OpenCL kernel execution based on the configuration parameters.
        ...
        for (auto r = 0; r < runs; r++) {
            try {
                // Execute the kernel and read the memory results.
                ...
                // Calculates the error compared to the referenced version and sum the execution times for all iterations.
                auto this_error =
        ...
        // Error less than the maximum limit and previous best execution time, updates the optinal parameters.
        if (error < getTunerMaxError<net_t>()
    ...
    // Handles the case in which no working configuration was found and return the optimal parameters.
    if (best_time == 0) {
        if (failed_compile > 0) {
...
// Memorize the optimize tuners for the SGEMM kernel.
template <typename net_t>
void Tuner<net_t>::store_sgemm_tuners(const int m, const int n, const int k,
                                      const int batch_size,
                                      std::string tuners) {
    // Obtain the tuner file paths.
...
std::string Tuner<net_t>::sgemm_tuners_from_line(std::string line, const int m,
    ...
    // Checks if the tuner version in the line corresponds to the current version.
    if (s[0] != std::to_string(TUNER_VERSION)) {
        return "";
    }
    
    // Checks if the parameters m, n, k and batch_size in the line correspond to the provided values (s[1] to s[5]).
    ...
    // Checks if the name of the OpenCL device in the line corresponds to the current device name.
