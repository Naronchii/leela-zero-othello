...
#ifdef HAVE_SELECT
    fd_set read_fds;
    FD_ZERO(&read_fds); // All bits reset to 0.
    FD_SET(0, &read_fds); // Sets the bit corresponding to the used input.
    struct timeval timeout{0, 0};
    select(1, &read_fds, nullptr, nullptr, &timeout);
    return FD_ISSET(0, &read_fds);
#else // Windows systems: OS doesn't support select().
...
static void gtp_fprintf(FILE* const file, const std::string& prefix,
    ...
    fprintf(file, "%s ", prefix.c_str()); // Prefix print.
    vfprintf(file, fmt, ap); // Prints the message from the specified file.
    fprintf(file, "\n\n");
}
...
void Utils::gtp_printf(const int id, const char* const fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    gtp_base_printf(id, "=", fmt, ap); // Print according to the GTP protocol specifications.
    va_end(ap);
}
...
std::string Utils::leelaz_file(const std::string& file) {
    ...
    const char* homedir;
    if ((homedir = getenv("HOME")) == nullptr) { // Use the user's home directory.
        struct passwd* pwd;
