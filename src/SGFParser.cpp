...
std::vector<std::string> SGFParser::chop_stream(std::istream& ins,
    ...
    bool intag = false; // Handles tags. Tags are opened with "[" and cannot contain any other parentheses before being closed with "]".
    int line = 0;
    gamebuff.clear();
    ...
    while (ins >> c && result.size() <= stopat) {
        ...
        // Guarantees that the characters after "\\" won't be read as special characters.
        if (c == '\\') {
            // read literal char
            ins >> c;
            gamebuff.push_back(c);
        ...
        // Handles "(" and ")". If we are not inside another set of round parentheses,
        // then white spaces and ";" will be ignored. They will be read in the while
        // cycle and then the gamebuff will be cleared.
        // This helps register only essential information.
        if (c == '(' && !intag) {
            if (nesting == 0) {
                // eat ; too
                do {
            ...
            // If nesting == 0, then the game has ended and will be saved in the result vector.
            if (nesting == 0) {
...
bool SGFParser::parse_property_value(std::istringstream& strm,
    ...
    // If c is not a white space, the character will be returned in the input stream.
    char c;
    while (strm >> c) {
        if (!std::isspace(c)) {
            strm.unget();
...
// Creates an SGF tree from an SGF input stream that represents the game's structure.
void SGFParser::parse(std::istringstream& strm, SGFTree* node) {
    ...
    while (strm >> c) {
        ...
        if (std::isalpha(c) && std::isupper(c)) {
            // The property is saved by the next character. The current one is discarded.
            strm.unget();
            ...
            do {
                ...
                // If the analysis of the value succeeds, then create a node that contains the property.
                if (success) {
        ...
        if (c == '(') {
            ...
            // Continue parsing the input of the new game variation, starting from the new node.
            parse(strm, newptr);
        } else if (c == ')') {
